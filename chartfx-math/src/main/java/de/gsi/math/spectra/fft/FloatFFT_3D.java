...
/**
 * Computes 3D Discrete Fourier Transform (DFT) of complex and real, single precision data. The sizes of all three
...
public strictfp class FloatFFT_3D {
     ...
     */
    public FloatFFT_3D(final int slices, final int rows, final int columns) {
    ...
    private void cdft3db_sub(final int isgn, final float[] a, final boolean scale) {
            ...
            if (nthreads > 1 && useThreads && slices >= nthreads && rows >= nthreads && columns >= nthreads) {
                ...
                for (int l = 0; l < nthreads; l++) {
                    ...
                    futures[l] = ConcurrencyUtils.submit(new Runnable() {
                        @Override
                        public void run() {
                            final float[] temp = new float[2 * slices];
        ...
        if (nthreads > 1 && useThreads && n1d2 >= nthreads && columns >= nthreads && ldimn2 >= nthreads) {
            ...
            for (int l = 0; l < nthreads; l++) {
                ...
                futures[l] = ConcurrencyUtils.submit(new Runnable() {
                    @Override
                    public void run() {
                        final float[] temp = new float[twon3];
                        for (int s = firstSlice; s >= lastSlice; s--) {
                            final int idx1 = s * sliceStride;
    ...
    private void rdft3d_sub(final int isgn, final float[] a) {
    ...
    private void xdft3da_sub1(final int icr, final int isgn, final float[] a, final boolean scale) {
        int idx0, idx1, idx2, idx3, idx4, idx5;
    ...
    private void xdft3da_sub2(final int icr, final int isgn, final float[] a, final boolean scale) {
        int idx0, idx1, idx2, idx3, idx4, idx5;
