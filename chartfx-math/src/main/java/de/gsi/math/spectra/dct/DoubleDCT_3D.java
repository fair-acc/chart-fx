...
/**
 * Computes 3D Discrete Cosine Transform (DCT) of double precision data. The sizes of all three dimensions can be
 * arbitrary numbers. This is a parallel implementation of split-radix and mixed-radix algorithms optimized for SMP
 * systems. <br>
 * <br>
...
public class DoubleDCT_3D {
    ...
    private final DoubleDCT_1D dctSlices;
     ...
     */
    public DoubleDCT_3D(final int slices, final int rows, final int columns) {
    ...
    /**
     * Computes the 3D forward DCT (DCT-II) leaving the result in <code>a</code> . The data is stored in 1D array
    ...
    /**
     * Computes the 3D forward DCT (DCT-II) leaving the result in <code>a</code> . The data is stored in 3D array
     *
     * @param a data to transform
     * @param scale if true then scaling is performed
     */
    ...
    /**
     * Computes the 3D inverse DCT (DCT-III) leaving the result in <code>a</code>. The data is stored in 1D array
     * addressed in slice-major, then row-major, then column-major, in order of significance, i.e. the element (i,j,k)
     * of 3D array x[slices][rows][columns] is stored in a[i*sliceStride + j*rowStride + k], where sliceStride = rows *
     * columns and rowStride = columns.
     *
    ...
    /**
     * Computes the 3D inverse DCT (DCT-III) leaving the result in <code>a</code>. The data is stored in 3D array.
     *
     * @param a data to transform
     * @param scale if true then scaling is performed
     */
