 ...
 *
 * The Initial Developer of the Original Code is Piotr Wendykier, Emory University. Portions created by the Initial
 * Developer are Copyright (C) 2007-2009 the Initial Developer. All Rights Reserved.
 *
...
/**
 * Computes 1D Discrete Fourier Transform (DFT) of complex and real, double precision data. The size of the data can be
...
public class DoubleFFT_1D {
    ...
    private static final double PI = TMath.Pi();
    private static final double TWO_PI = TMath.TwoPi();
     ...
     */
    public DoubleFFT_1D(final int n) {
        if (n < 1) {
        ...
        if (!ConcurrencyUtils.isPowerOf2(n)) {
            if (getReminder(n, factors) >= 211) {
                ...
                ip = new int[2 + (int) Math.ceil(2 + (1 << (int) (Math.log(nBluestein + 0.5) / Math.log(2)) / 2))];
            ...
            ip = new int[2 + (int) Math.ceil(2 + (1 << (int) (Math.log(n + 0.5) / Math.log(2)) / 2))];
            w = new double[n];
            final int twon = 2 * n;
            nw = ip[0];
            if (twon > nw << 2) {
    ...
    /**
     * Computes 1D forward DFT of complex data leaving the result in <code>a</code>. Complex number is stored as two
     * double values in sequence: the real and imaginary part, i.e. the size of the input array must be greater or equal
     * 2*n. The physical layout of the input data has to be as follows:<br>
     *
     * <pre>
     * a[2*k] = Re[k],
     * a[2*k+1] = Im[k], 0&lt;=k&lt;n
     ...
     */
    public void complexForward(final double[] a, final int offa) {
        if (n == 1) {
            return;
        }
        switch (plan) {
    ...
    /**
     * Computes 1D inverse DFT of complex data leaving the result in <code>a</code>. Complex number is stored as two
     * double values in sequence: the real and imaginary part, i.e. the size of the input array must be greater or equal
     * 2*n. The physical layout of the input data has to be as follows:<br>
     *
     * <pre>
     ...
     * @param scale if true then scaling is performed
     */
    public void complexInverse(final double[] a, final int offa, final boolean scale) {
        if (n == 1) {
            return;
        }
        switch (plan) {
    ...
    /**
     * Computes 1D forward DFT of real data leaving the result in <code>a</code>. The physical layout of the output data
     * is as follows:<br>
     * if n is even then
     *
     * <pre>
     ...
     * @param offa index of the first element in array <code>a</code>
     */
    public void realForward(final double[] a, final int offa) {
        if (n == 1) {
            return;
        }
    ...
    public void realForwardFull(final double[] a, final int offa) {
        ...
        case MIXED_RADIX:
            ...
            for (int k = 1; k < m; k++) {
                final int idx1 = offa + twon - 2 * k;
                final int idx2 = offa + 2 * k;
                a[idx1 + 1] = -a[idx2];
                a[idx1] = a[idx2 - 1];
            }
            for (int k = 1; k < n; k++) {
                final int idx = offa + n - k;
                final double tmp = a[idx + 1];
                a[idx + 1] = a[idx];
                a[idx] = tmp;
            }
            a[offa + 1] = 0;
     ...
     * @param scale if true then scaling is performed
     */
    public void realInverse(final double[] a, final int offa, final boolean scale) {
        if (n == 1) {
            return;
        }
        switch (plan) {
     ...
     * @param scale if true then scaling is performed
     */
    public void realInverseFull(final double[] a, final int offa, final boolean scale) {
        final int twon = 2 * n;
        switch (plan) {
        case SPLIT_RADIX:
        ...
        case MIXED_RADIX:
            ...
            for (int k = 1; k < m; k++) {
                final int idx1 = offa + 2 * k;
                final int idx2 = offa + twon - 2 * k;
                a[idx1] = -a[idx1];
                a[idx2 + 1] = -a[idx1];
                a[idx2] = a[idx1 - 1];
            }
    ...
    protected void realInverse2(final double[] a, final int offa, final boolean scale) {
        if (n == 1) {
            return;
        }
        switch (plan) {
    ...
    void cffti(final int n, final int offw) {
        if (n == 1) {
            return;
        }
    ...
    private void bluesteini() {
        ...
        final double scale = 1.0 / nBluestein;
        bk2[0] = bk1[0] * scale;
        bk2[1] = bk1[1] * scale;
                    ...
                    public void run() {
                        if (isign > 0) {
                            for (int i = firstIdx; i < lastIdx; i++) {
                                final int idx1 = 2 * i;
                                final int idx2 = idx1 + 1;
                                final double im = -ak[idx1] * bk2[idx2] + ak[idx2] * bk2[idx1];
    ...
    private void bluestein_real_inverse(final double[] a, final int offa) {
        ...
        if (n % 2 == 0) {
            ...
            for (int i = n / 2 + 1; i < n; i++) {
                final int idx1 = 2 * i;
                final int idx2 = idx1 + 1;
                final int idx3 = offa + 2 * n - idx1;
                final int idx4 = idx3 + 1;
                ak[idx1] = a[idx3] * bk1[idx1] + a[idx4] * bk1[idx2];
                ak[idx2] = a[idx3] * bk1[idx2] - a[idx4] * bk1[idx1];
            }
    ...
    void radf2(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off,
        ...
        for (int k = 0; k < l1; k++) {
            ...
            final double i1r = in[iidx1];
            final double i2r = in[iidx2];
        ...
        if (ido != 2) {
            for (int k = 0; k < l1; k++) {
                ...
                for (i = 2; i < ido; i += 2) {
                    ...
                    final double a1i = in[iidx1 - 1];
                    final double a1r = in[iidx1];
                    final double a2i = in[iidx2 - 1];
                    final double a2r = in[iidx2];
    ...
    void radb2(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off,
        ...
        for (int k = 0; k < l1; k++) {
            ...
            final int idx2 = 2 * idx1;
            final int idx3 = idx2 + ido;
            final int oidx1 = out_off + idx1;
            final int iidx1 = in_off + idx2;
            final int iidx2 = in_off + ido - 1 + idx3;
            final double i1r = in[iidx1];
            final double i2r = in[iidx2];
            out[oidx1] = i1r + i2r;
        ...
        if (ido != 2) {
            for (int k = 0; k < l1; ++k) {
                ...
                final int idx2 = 2 * idx1;
                final int idx3 = idx2 + ido;
                final int idx4 = idx1 + idx0;
                for (i = 2; i < ido; i += 2) {
                    ic = ido - i;
                    final int idx5 = i - 1 + iw1;
                    final int idx6 = out_off + i;
                    final int idx7 = in_off + i;
                    final int idx8 = in_off + ic;
                    w1r = wtable_r[idx5 - 1];
                    w1i = wtable_r[idx5];
                    final int iidx1 = idx7 + idx2;
                    final int iidx2 = idx8 + idx3;
                    final int oidx1 = idx6 + idx1;
                    final int oidx2 = idx6 + idx4;
                    t1r = in[iidx1 - 1] - in[iidx2 - 1];
                    t1i = in[iidx1] + in[iidx2];
                    final double i1i = in[iidx1];
                    final double i1r = in[iidx1 - 1];
                    final double i2i = in[iidx2];
                    final double i2r = in[iidx2 - 1];
        ...
        for (int k = 0; k < l1; k++) {
            final int idx1 = k * ido;
            final int idx2 = 2 * idx1;
            final int oidx1 = out_off + ido - 1 + idx1;
            final int iidx1 = in_off + idx2 + ido;
    ...
    void radf3(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off,
        ...
        for (int k = 0; k < l1; k++) {
            ...
            for (i = 2; i < ido; i += 2) {
                ...
                final double i1i = in[iidx1 - 1];
                final double i1r = in[iidx1];
                final double i2i = in[iidx2 - 1];
                final double i2r = in[iidx2];
                final double i3i = in[iidx3 - 1];
                final double i3r = in[iidx3];
    ...
    void radb3(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off,
        ...
        for (int k = 0; k < l1; k++) {
            final int idx1 = k * ido;
            final int iidx1 = in_off + 3 * idx1;
            final int iidx2 = iidx1 + 2 * ido;
            final double i1i = in[iidx1];
        ...
        for (int k = 0; k < l1; k++) {
            final int idx1 = k * ido;
            final int idx2 = 3 * idx1;
            final int idx3 = idx2 + ido;
            final int idx4 = idx3 + ido;
            final int idx5 = idx1 + idx0;
            final int idx6 = idx5 + idx0;
            for (i = 2; i < ido; i += 2) {
                ic = ido - i;
                final int idx7 = in_off + i;
                final int idx8 = in_off + ic;
                final int idx9 = out_off + i;
                final int iidx1 = idx7 + idx2;
                final int iidx2 = idx7 + idx4;
                final int iidx3 = idx8 + idx3;
                ...
                final int oidx1 = idx9 + idx1;
                final int oidx2 = idx9 + idx5;
                final int oidx3 = idx9 + idx6;
    ...
    void radf4(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off,
        ...
        for (int k = 0; k < l1; k++) {
            ...
            final double i1r = in[in_off + idx1];
            final double i2r = in[in_off + idx3];
            final double i3r = in[in_off + idx4];
            final double i4r = in[in_off + idx5];
        ...
        if (ido != 2) {
            for (int k = 0; k < l1; k++) {
                ...
                for (i = 2; i < ido; i += 2) {
                    ...
                    final double i3i = in[iidx3 - 1];
                    final double i3r = in[iidx3];
                    final double i4i = in[iidx4 - 1];
                    final double i4r = in[iidx4];
        ...
        for (int k = 0; k < l1; k++) {
            ...
            final double i1i = in[idx9 - 1 + idx1];
            final double i2i = in[idx9 - 1 + idx3];
            final double i3i = in[idx9 - 1 + idx4];
            final double i4i = in[idx9 - 1 + idx5];
    ...
    void radb4(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off,
        ...
        for (int k = 0; k < l1; k++) {
            ...
            final double i1r = in[in_off + idx2];
            final double i2r = in[in_off + idx7];
            final double i3r = in[in_off + ido - 1 + idx8];
            final double i4r = in[in_off + ido - 1 + idx6];
        ...
        if (ido != 2) {
            for (int k = 0; k < l1; ++k) {
                ...
                for (i = 2; i < ido; i += 2) {
                    ...
                    final int idx12 = in_off + i;
                    final int idx13 = in_off + ic;
                    final int idx14 = out_off + i;

                    final int iidx1 = idx12 + idx5;
                    final int iidx2 = idx13 + idx6;
                    final int iidx3 = idx12 + idx7;
                    final int iidx4 = idx13 + idx8;
                    ...
                    final int oidx1 = idx14 + idx1;
                    final int oidx2 = idx14 + idx2;
                    final int oidx3 = idx14 + idx3;
                    final int oidx4 = idx14 + idx4;
        ...
        for (int k = 0; k < l1; k++) {
            ...
            final double i1r = in[idx9 - 1 + idx2];
            final double i2r = in[idx9 - 1 + idx7];
            final double i3r = in[in_off + idx6];
            final double i4r = in[in_off + idx8];
    ...
    void radf5(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off,
        ...
        for (int k = 0; k < l1; k++) {
            ...
            final double i1r = in[in_off + idx1];
            final double i2r = in[in_off + idx7];
            final double i3r = in[in_off + idx8];
            final double i4r = in[in_off + idx9];
            final double i5r = in[in_off + idx10];
        ...
        for (int k = 0; k < l1; ++k) {
            ...
            for (i = 2; i < ido; i += 2) {
                ...
                final double i4i = in[iidx4 - 1];
                final double i4r = in[iidx4];
                final double i5i = in[iidx5 - 1];
                final double i5r = in[iidx5];
    ...
    void radb5(final int ido, final int l1, final double in[], final int in_off, final double out[], final int out_off,
        ...
        for (int k = 0; k < l1; ++k) {
            ...
            for (i = 2; i < ido; i += 2) {
                ...
                final int idx15 = in_off + i;
                final int idx16 = in_off + ic;
                final int idx17 = out_off + i;

                final int iidx1 = idx15 + idx2;
                final int iidx2 = idx16 + idx3;
                final int iidx3 = idx15 + idx4;
                final int iidx4 = idx16 + idx5;
                final int iidx5 = idx15 + idx6;
                ...
                final int oidx1 = idx17 + idx1;
                final int oidx2 = idx17 + idx7;
                final int oidx3 = idx17 + idx8;
                final int oidx4 = idx17 + idx9;
                final int oidx5 = idx17 + idx10;
                ...
                for (int j = 1; j < ipph; j++) {
                    ...
                    for (int k = 0; k < l1; k++) {
                        ...
                        for (int i = 2; i < ido; i += 2) {
                            ...
                            final int oidx2 = idx6 + idx4;
                            final double o1i = out[oidx1 - 1];
                            final double o1r = out[oidx1];
                            final double o2i = out[oidx2 - 1];
                            final double o2r = out[oidx2];
    ...
    void radbg(final int ido, final int ip, final int l1, final int idl1, final double in[], final int in_off,
        ...
        nbd = (ido - 1) / 2;
        ipph = (ip + 1) / 2;
        final int idx0 = ip * ido;
        if (ido >= l1) {
            for (int k = 0; k < l1; k++) {
                final int idx1 = k * ido;
                final int idx2 = k * idx0;
                for (int i = 0; i < ido; i++) {
            ...
            for (int i = 0; i < ido; i++) {
                final int idx1 = out_off + i;
                final int idx2 = in_off + i;
                for (int k = 0; k < l1; k++) {
                    out[idx1 + k * ido] = in[idx2 + k * idx0];
                }
            }
        }
        final int iidx0 = in_off + ido - 1;
        for (int j = 1; j < ipph; j++) {
            jc = ip - j;
            j2 = 2 * j;
            final int idx1 = j * l1 * ido;
            ...
            for (int k = 0; k < l1; k++) {
                final int idx4 = k * ido;
                final int idx5 = idx4 * ip;
                final int iidx1 = iidx0 + idx3 + idx5 - ido;
                final int iidx2 = in_off + idx3 + idx5;
                final double i1r = in[iidx1];
                final double i2r = in[iidx2];
        ...
        if (ido != 1) {
            if (nbd >= l1) {
                for (int j = 1; j < ipph; j++) {
                    ...
                    final int idx1 = j * l1 * ido;
                    final int idx2 = jc * l1 * ido;
                    final int idx3 = 2 * j * ido;
                    for (int k = 0; k < l1; k++) {
                        final int idx4 = k * ido + idx1;
                        final int idx5 = k * ido + idx2;
                        final int idx6 = k * ip * ido + idx3;
                        for (int i = 2; i < ido; i += 2) {
                            ic = ido - i;
                            final int idx7 = out_off + i;
                            final int idx8 = in_off + ic;
                            final int idx9 = in_off + i;
                            final int oidx1 = idx7 + idx4;
                            final int oidx2 = idx7 + idx5;
                            final int iidx1 = idx9 + idx6;
                            final int iidx2 = idx8 + idx6 - ido;
                            final double a1i = in[iidx1 - 1];
                            final double a1r = in[iidx1];
                            final double a2i = in[iidx2 - 1];
                            final double a2r = in[iidx2];
        ...
        for (int l = 1; l < ipph; l++) {
            ...
            final int idx2 = lc * idl1;
            for (int ik = 0; ik < idl1; ik++) {
                final int idx3 = in_off + ik;
                final int idx4 = out_off + ik;
            ...
            for (int j = 2; j < ipph; j++) {
                ...
                ai2 = dc2 * ai2 + ds2 * ar2;
                ar2 = ar2h;
                final int idx5 = j * idl1;
                final int idx6 = jc * idl1;
                for (int ik = 0; ik < idl1; ik++) {
                    final int idx7 = in_off + ik;
                    final int idx8 = out_off + ik;
        ...
        for (int j = 1; j < ipph; j++) {
            ...
            for (int k = 0; k < l1; k++) {
                final int idx3 = k * ido;
                final int oidx1 = out_off + idx3;
                final int iidx1 = in_off + idx3 + idx1;
                final int iidx2 = in_off + idx3 + idx2;
                final double i1r = in[iidx1];
                final double i2r = in[iidx2];
            ...
            for (int j = 1; j < ipph; j++) {
                ...
                for (int k = 0; k < l1; k++) {
                    ...
                    for (int i = 2; i < ido; i += 2) {
                        ...
                        final int idx5 = in_off + i;
                        final int oidx1 = idx4 + idx3 + idx1;
                        final int oidx2 = idx4 + idx3 + idx2;
                        final int iidx1 = idx5 + idx3 + idx1;
                        final int iidx2 = idx5 + idx3 + idx2;
                        final double i1i = in[iidx1 - 1];
                        final double i1r = in[iidx1];
                        final double i2i = in[iidx2 - 1];
                        final double i2r = in[iidx2];
    ...
    /*---------------------------------------------------------
      ...
      --------------------------------------------------------*/
    void cfftf(final double a[], final int offa, final int isign) {
        int idot;
        int l1, l2;
        ...
        int iw1, iw2;
        final double[] ch = new double[twon];
        ...
        if (ido <= 2) {
            ...
            for (int k = 0; k < l1; k++) {
                for (int i = 0; i < ido - 1; i += 2) {
                    ...
                    final double w1r = wtable[widx1];
                    final double w1i = isign * wtable[widx1 + 1];
        ...
        if (ido == 2) {
            ...
            for (int k = 1; k <= l1; k++) {
                ...
                for (int i = 0; i < ido - 1; i += 2) {
                    ...
                    final double a2r = in[iidx2];
                    final double a2i = in[iidx2 + 1];
                    final double a3r = in[iidx3];
                    final double a3i = in[iidx3 + 1];
                    ...
                    final double w1r = wtable[widx1];
                    final double w1i = isign * wtable[widx1 + 1];
                    final double w2r = wtable[widx2];
                    final double w2i = isign * wtable[widx2 + 1];
        ...
        if (ido == 2) {
            ...
            for (int k = 0; k < l1; k++) {
                ...
                for (int i = 0; i < ido - 1; i += 2) {
                    ...
                    final double w2r = wtable[widx2];
                    final double w2i = isign * wtable[widx2 + 1];
                    final double w3r = wtable[widx3];
                    final double w3i = isign * wtable[widx3 + 1];
        ...
        if (ido == 2) {
            ...
            for (int k = 1; k <= l1; k++) {
                ...
                for (int i = 0; i < ido - 1; i += 2) {
                    ...
                    final double w3r = wtable[widx3];
                    final double w3i = isign * wtable[widx3 + 1];
                    final double w4r = wtable[widx4];
                    final double w4i = isign * wtable[widx4 + 1];
            ...
            for (int j = 1; j < ipph; j++) {
                ...
                for (int k = 0; k < l1; k++) {
                    ...
                    for (int i = 0; i < ido; i++) {
                        ...
                        final double i1r = in[in_off + i + idx1 + idx6];
                        final double i2r = in[in_off + i + idx2 + idx6];
            ...
            for (int j = 1; j < ipph; j++) {
                ...
                for (int i = 0; i < ido; i++) {
                    for (int k = 0; k < l1; k++) {
                        ...
                        final int idx4 = in_off + i;
                        final double i1r = in[idx4 + idxt3 + idx2];
                        final double i2r = in[idx4 + idxt4 + idx2];
    ...
    private void cftxb020(final double[] a, final int offa) {
        double x0r, x0i;
    ...
    private void cftxc020(final double[] a, final int offa) {
        double x0r, x0i;
        x0r = a[offa] - a[offa + 2];
        x0i = a[offa + 1] + a[offa + 3];
        a[offa] += a[offa + 2];
    ...
    private void scale(final double m, final double[] a, final int offa, final boolean complex) {
    ...
    /**
     * @return dimension this class has been initialised for
     */
