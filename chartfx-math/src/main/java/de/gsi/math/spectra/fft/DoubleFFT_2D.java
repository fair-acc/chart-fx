...
/**
 * Computes 2D Discrete Fourier Transform (DFT) of complex and real, double precision data. The sizes of both dimensions
...
public class DoubleFFT_2D {
     ...
     */
    public DoubleFFT_2D(final int rows, final int columns) {
        if (rows <= 1 || columns <= 1) {
    ...
    private void cdft2d_sub(final int isgn, final double[] a, final boolean scale) {
     ...
     * must be of size rows by 2*columns. The physical layout of the input data has to be as follows:<br>
     *
     * <pre>
     * a[k1][2*k2] = Re[k1][k2],
    ...
    private void rdft2d_sub(final int isgn, final double[] a) {
        int n1h, j;
        double xi;
        int idx1, idx2;
