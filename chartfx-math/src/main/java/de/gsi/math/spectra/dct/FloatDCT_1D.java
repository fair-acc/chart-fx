...
/**
 * Computes 1D Discrete Cosine Transform (DCT) of single precision data. The size of data can be an arbitrary number.
...
public class FloatDCT_1D {
     ...
     */
    public FloatDCT_1D(final int n) {
        if (n < 1) {
    ...
    private void dctsub(final int n, final float[] a, final int offa, final int nc, final float[] c, final int startc) {
        int k, kk, ks, m;
        float wkr, wki, xr;
        int idx0, idx1, idx2;
    ...
    private float[] makect(final int n) {
        final int twon = 2 * n;
        int idx;
        final double delta = PI / twon;
        double deltaj;
        final float[] c = new float[twon];
        c[0] = 1;
        for (int j = 1; j < n; j++) {
            idx = 2 * j;
            deltaj = delta * j;
