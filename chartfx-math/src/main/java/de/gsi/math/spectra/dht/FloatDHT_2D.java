...
/**
 * Computes 2D Discrete Hartley Transform (DHT) of real, single precision data. The sizes of both dimensions can be
...
public class FloatDHT_2D {
     ...
     */
    public FloatDHT_2D(final int rows, final int column) {
        if (rows <= 1 || column <= 1) {
    ...
    private void y_transform(final float[][] a) {
        int mRow, mCol;
        float A, B, C, D, E;
