...
public class FloatFFT2DTests {
    ...
    public void basicComplex2dIdentityTests(final int nRows, final int nCols) {
        FloatFFT_2D fft = new FloatFFT_2D(nRows, nCols);

        final int nSamples = 4 * nRows * nCols;
        float[][] testSignal1Ref = generateDelta(2 * nRows, 2 * nCols);
        float[][] testSignal1 = generateDelta(2 * nRows, 2 * nCols);
    ...
    public void identityFullRealFFTTests(final int nSamples, final boolean forward) {
        FloatFFT_2D fft = new FloatFFT_2D(nSamples, nSamples);

        float[] testSignal1a = generateDelta(2 * nSamples * nSamples);
        float[] testSignal1b = generateDelta(2 * nSamples * nSamples);
    ...
    public void identityFullRealFFTTests2(final int nSamples, final boolean forward) {
        ...
        float[][] testSignal1a = generateDelta(nSamples, 2 * nSamples);
        float[][] testSignal1b = generateDelta(nSamples, 2 * nSamples);
