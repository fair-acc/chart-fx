...
import de.gsi.chart.data.EditConstraints;
import de.gsi.chart.data.EditableDataSet;
...
public class EditDataSet extends TableViewer {
     ...
     */
    protected EventHandler<MouseEvent> startDragHandler(SelectedDataPoint node) {
        return event -> {
     ...
     */
    private Pair<DataPoint, DataPoint> findNeighborPoints(final DataSet series, final double searchedX) {
        int prevIndex = -1;
        int nextIndex = -1;
        double prevX = Double.MIN_VALUE;
        double nextX = Double.MAX_VALUE;

        for (int i = 0, size = series.getDataCount(); i < size; i++) {
            final double currentX = series.getX(i);
        ...
        final DataPoint prevPoint = prevIndex == -1 ? null
                : new DataPoint(getChart(), series, prevIndex, series.getX(prevIndex), series.getY(prevIndex),
                        series.getDataLabel(prevIndex));
        final DataPoint nextPoint = nextIndex == -1 || nextIndex == prevIndex ? null
                : new DataPoint(getChart(), series, nextIndex, series.getX(nextIndex), series.getY(nextIndex),
                        series.getDataLabel(nextIndex));
