 ...
 * BEGIN_HTML <!----> <A href="ftp://root.cern.ch/root/Spectrum.doc">Spectrum.doc</A><br>
 ...
 * <!----> END_HTML The original source documentation as well as examples can be found at <A
 * href="http://root.cern.ch/</A>
 */
...
public class TSpectrum {
     ...
     *            distance between peaks. Higher values allow higher resolution (smaller distance between peaks. May be
     *            set later through SetResolution.)
     */
     ...
     *            "BackOrder2" -possible values= "BackOrder4" "BackOrder6" "BackOrder8" - "nosmoothing"- if selected,
     *            the background is not smoothed By default the background is smoothed. - smoothWindow-width of
     *            smoothing window, (default is "BackSmoothing3") -possible values= "BackSmoothing5" "BackSmoothing7"
     *            "BackSmoothing9" "BackSmoothing11" "BackSmoothing13" "BackSmoothing15" - "Compton" if selected the
     *            estimation of Compton edge will be included. NOTE that the background is only evaluated in the current
     *            range of h. ie, if h has a bin range (set via h->GetXaxis()->SetRange(binmin,binmax), the returned
     *            histogram will be created with the same number of bins as the input histogram h, but only bins from
     *            binmin to binmax will be filled with the estimated background.
     * @return data set
     */
     ...
     * @parame threshold: (default=0.05) peaks with amplitude less than threshold*highest_peak are discarded.
