...
public final class ArrayMath {
     ...
     * @param sampleRate
     *            the sample rate
     * @param cutoffFraction
     *            cutoffFreq must be smaller than half the samplerate
     * @param filterOrder
     *            can be between 2 and 20
     * @param filterType
     *            0: lowPass 1: highPass
     * @param ripplePercent
     *            ripplePercent is amount of ripple in Chebyshev filter (0-29)
     *            (ripplePercent == 0 -> Butterworth)
     ...
     */
    /**
     * calculate 2nd-order filter with Butterworth (rippleP=0) or Chebychev
     * filter response (rippleP>0)
     * N.B. numerically validated against data set examples in Table 20-6
     ...
     * @param rippleP
     *            >0 -> Chebychev design-ripple in percent in the pass-band
     * @param numberOfPoles
     *            number of poles (if combined with 2*n filter stages)
     * @param iteration
     *            (whether this is the i-th filter in the chain)
