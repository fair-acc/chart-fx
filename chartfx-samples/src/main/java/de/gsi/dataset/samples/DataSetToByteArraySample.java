...
public class DataSetToByteArraySample {
    ...
    public DataSetToByteArraySample() {
        ...
        DataSetUtils.writeDataSetToByteArray(original, byteOutput, false);
        System.err.println("byte buffer array length with string encoding = " + byteOutput.size());

        DataSetUtils.writeDataSetToByteArray(original, byteOutput, true);
        System.err.println("byte buffer array length with binary encoding = " + byteOutput.size());
    }

    public void testIdentity(final boolean binary) {
        DataSetUtils.writeDataSetToByteArray(original, byteOutput, binary);
        ...
        for (int i = 0; i < original.getDataCount(); i++) {
            ...
            if ((x0 != x1) || (y0 != y1) || (exn0 != exn1) || (exp0 != exp1) || (eyn0 != eyn1) || (eyp0 != eyp1)) {
                throw new IllegalStateException("data set name do not match (" + binary + "): original-copy = \n"
                        ...
                        + String.format("diffs (dx=%e, dy=%e, dexn=%e, dexp=%e, deyn=%e, deyp=%e)", x0 - x1, y0 - y1,
        ...
        System.err.println("all identity tests passed for " + (binary ? "binary" : "string") + "-based encoding");
    }
    ...
    public void testPerformance(final int iterations, final boolean binary) {
        ...
        System.err.println("average " + (binary ? "binary" : "string") + "-encoded throughput = "
                + humanReadableByteCount((long)byteCount, true) +"/s");
    }
    ...
    public static void main(final String[] args) {
        ...
        sample.testPerformance(10, false);
        sample.testPerformance(10, true);

        sample.testIdentity(true);
        sample.testIdentity(false);
