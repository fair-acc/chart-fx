...
public class HistogramRendererBarSample extends ChartSample {
    ...
    private final Map<String, Map<String, Map<Integer, io.fair_acc.dataset.Histogram>>> relDistribution = new ConcurrentHashMap<>(); // Map<country name, Map<men/women, Map<year,Histogram>>>
    private final Map<String, Map<String, Map<Integer, io.fair_acc.dataset.Histogram>>> absDistribution = new ConcurrentHashMap<>(); // Map<country name, Map<men/women, Map<year,Histogram>>>
    ...
    private void loadDemoData(final String fileName) {
        try (BufferedReader csvReader = Files.newBufferedReader(Paths.get(Objects.requireNonNull(this.getClass().getResource(fileName)).toURI()))) {
            ...
            while ((row = csvReader.readLine()) != null) {
                ...
                final Map<String, Map<Integer, io.fair_acc.dataset.Histogram>> relCountryMap = relDistribution.computeIfAbsent(country, c -> new ConcurrentHashMap<>());
                final Map<Integer, io.fair_acc.dataset.Histogram> relWomenDistribution = relCountryMap.computeIfAbsent(WOMEN, g -> new ConcurrentHashMap<>());
                final Map<Integer, io.fair_acc.dataset.Histogram> relMenDistribution = relCountryMap.computeIfAbsent(MEN, g -> new ConcurrentHashMap<>());
                ...
                final Map<String, Map<Integer, io.fair_acc.dataset.Histogram>> absCountryMap = absDistribution.computeIfAbsent(country, c -> new ConcurrentHashMap<>());
                final Map<Integer, io.fair_acc.dataset.Histogram> absWomenDistribution = absCountryMap.computeIfAbsent(WOMEN, g -> new ConcurrentHashMap<>());
                final Map<Integer, io.fair_acc.dataset.Histogram> absMenDistribution = absCountryMap.computeIfAbsent(MEN, g -> new ConcurrentHashMap<>());
    ...
    public static class MyHistogram extends Histogram {
