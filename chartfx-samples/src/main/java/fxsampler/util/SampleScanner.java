 ...
 * <p>
 * NOTE:
 * Edited from org.controlsfx:fxsampler:1.0.10 to support working
 * in Conveyor packaged releases. The default scanner only searches
 * in paths at or below the working directory, but in Conveyor the
 * jars are in a sibling folder, so the scanner doesn't find anything.
 * TODO: remove when the upstream version works
 */
...
//            if (Sample.class.isAssignableFrom(EmptySample.class)) continue;
    ...
    private Class<?>[] loadFromPathScanning() throws ClassNotFoundException, IOException {
        ...
        if (appDir == null) {
            ...
            // NOTE: these are the only changes in this file
            System.out.println("Found conveyor package. Scanning jars in app dir: " + appDir);
            workingDirectory = Path.of(appDir).toAbsolutePath();
        }
