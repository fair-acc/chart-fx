...
public class Hexagon extends Polygon {
    ...
    public void drawContour(GraphicsContext gc) {
        ...
        gc.setFill(getFill());
        if (map == null) {
            drawHexagon(gc, HexagonMap.Direction.values());
            gc.restore();
        }

        final List<HexagonMap.Direction> list = new ArrayList<>();
        for (final HexagonMap.Direction direction : HexagonMap.Direction.values()) {
        ...
        drawHexagon(gc, list.toArray(new HexagonMap.Direction[0]));
    ...
    public void drawHexagon(final GraphicsContext gc, HexagonMap.Direction... directions) {
        final ObservableList<Double> points = getPoints();
        final int nPoints = points.size() / 2;
        final double[] xPoints = new double[nPoints];
        final double[] yPoints = new double[nPoints];
        ...
        for (final HexagonMap.Direction side : directions) {
            switch (side) {
            case EAST:
