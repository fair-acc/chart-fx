...
public class ContourDataSetRenderer extends AbstractContourDataSetRendererParameter<ContourDataSetRenderer>
    ...
    private void drawHexagonHeatMap(final GraphicsContext gc, final ContourDataSetCache lCache, boolean test) {
        final long start = ProcessingProfiler.getTimeStamp();
    ...
    private void drawHexagonMapContour(final GraphicsContext gc, final ContourDataSetCache lCache, boolean test) {
        final long start = ProcessingProfiler.getTimeStamp();
    ...
    private void paintCanvas(final GraphicsContext gc) {
        ...
        case CONTOUR_HEXAGON:
            if (isAltImplementation()) {
                drawHexagonMapContour(gc, localCache, true);
            } else {
                drawHexagonMapContourAlt(gc, axisTransform, localCache);
            }
            break;
        case HEATMAP_HEXAGON:
            drawHexagonHeatMap(gc, localCache, true);
            break;
        case HEATMAP:
        default:
            if (isAltImplementation()) {
