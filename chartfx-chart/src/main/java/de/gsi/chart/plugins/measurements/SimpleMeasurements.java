...
public class SimpleMeasurements extends ValueIndicator {
  private static final Logger LOGGER =
      LoggerFactory.getLogger(SimpleMeasurements.class);
  ...
  public SimpleMeasurements(final XYChart chart,
                            final MeasurementType measType) {
    super(chart, X);
    this.measType = measType;
    title = new StringBuilder()
                .append(measType)
                .append(" [#")
                .append(AbstractChartMeasurement.markerCount - 1)
                .append(", #")
                .append(AbstractChartMeasurement.markerCount)
                .append("]")
                .toString();
    valueField.setMinRange(SimpleMeasurements.DEFAULT_MIN)
        .setMaxRange(SimpleMeasurements.DEFAULT_MAX);
    ...
    if (measType != VALUE_HOR && measType != VALUE_VER) {
      ...
      sliderIndicator2.setText("Marker#" +
  ...
  public void handle(final UpdateEvent event) {
    ...
    case RISETIME_10_90:
      val =
          SimpleDataSetEstimators.getSimpleRiseTime1090(ds, indexMin, indexMax);
      break;
    case RISETIME_20_80:
      val =
          SimpleDataSetEstimators.getSimpleRiseTime2080(ds, indexMin, indexMax);
      break;
    case FWHM:
      val = SimpleDataSetEstimators.getFullWidthHalfMaximum(ds, indexMin,
                                                            indexMax, false);
      break;
    ...
    case LOCATION_MAXIMUM:
      val = selectedDataSet.get(
          DataSet.DIM_X,
          SimpleDataSetEstimators.getLocationMaximum(ds, indexMin, indexMax));
      break;
    case LOCATION_MAXIMUM_GAUSS:
      val = SimpleDataSetEstimators.getLocationMaximumGaussInterpolated(
          ds, indexMin, indexMax);
      break;
    case DUTY_CYCLE:
      val = SimpleDataSetEstimators.getDutyCycle(ds, indexMin, indexMax);
      break;
    case PERIOD:
      val = 1.0 / SimpleDataSetEstimators.getFrequencyEstimate(ds, indexMin,
                                                               indexMax);
      break;
    case FREQUENCY:
      val =
          SimpleDataSetEstimators.getFrequencyEstimate(ds, indexMin, indexMax);
      break;
    ...
    final Axis axis =
        measType.isVerticalMeasurement() ? chart.getYAxis() : chart.getXAxis();
    final Axis altAxis =
        measType.isVerticalMeasurement() ? chart.getXAxis() : chart.getYAxis();
    ...
    if (axis instanceof DefaultNumericAxis && axisUnit != null) {
      valueField.setUnit(
          new StringBuilder().append(axisPrefix).append(unit).toString());
      final AxisLabelFormatter axisFormatter =
          ((DefaultNumericAxis)axis).getAxisLabelFormatter();
      valueLabel = axisFormatter.toString(scaledValue);
    } else {
      if (Math.abs(Math.log10(Math.abs(val))) <
          ValueIndicator.SMALL_FORMAT_THRESHOLD) {
    ...
    default:
      final String unit2 =
          altAxisLabel.replaceAll("\\[", "").replaceAll("\\]", "");
  ...
  public void initialize() {
    final Node node = Borders.wrap(valueField)
                          .lineBorder()
                          .title(title)
                          .color(Color.BLACK)
                          .build()
  ...
  public enum MeasurementCategory {
    ...
    MeasurementCategory(final String description) { name = description; }
  ...
  public enum MeasurementType {
    ...
    MeasurementType(final boolean isVerticalMeasurement,
                    final MeasurementCategory measurementCategory,
    ...
    public boolean isVerticalMeasurement() { return isVertical; }
