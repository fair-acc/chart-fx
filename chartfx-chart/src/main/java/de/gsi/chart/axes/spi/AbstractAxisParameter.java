...
public abstract class AbstractAxisParameter extends Pane implements Axis {
    ...
    protected void updateAxisLabelAndUnit() {
        ...
        if ((localAxisUnit == null || localAxisUnit.trim().length()==0 || localAxisUnit.trim().length()==0) && !(axisPrefix == null || axisPrefix.trim().length()==0 || axisPrefix.trim().length()==0)) {
            localAxisUnit = "";
        }
    ...
    private static class StyleableProperties {
        private static final CssMetaData<AbstractAxisParameter, String> AXIS_LABEL = new CssMetaData<AbstractAxisParameter, String>("-fx-axis-label",
                com.sun.javafx.css.converters.StringConverter.getInstance()) {
            @Override
        ...
        private static final CssMetaData<AbstractAxisParameter, String> AXIS_UNIT = new CssMetaData<AbstractAxisParameter, String>("-fx-axis-unit",
                com.sun.javafx.css.converters.StringConverter.getInstance()) {
            @Override
            public StyleableProperty<String> getStyleableProperty(final AbstractAxisParameter n) {
