...
public abstract class AbstractAxis
    extends AbstractAxisParameter implements Axis {
   ...
   * be the same as lowerBound or different. It is used by NumberAxis to animate
   * the lowerBound from the old value to the new value.
   */
  protected final DoubleProperty currentLowerBound =
      new SimpleDoubleProperty(this, "currentLowerBound");

  private final ObjectProperty<AxisLabelFormatter> axisFormatter =
      new SimpleObjectProperty<>(this, "axisLabelFormatter", null) {
        /**
         * default fall-back formatter in case no {@code axisFormatter} is
         * specified (ie. 'null')
         */
        private final AxisLabelFormatter defaultFormatter =
            new DefaultFormatter(AbstractAxis.this);
        private final AxisLabelFormatter defaultLogFormatter =
            new DefaultLogFormatter(AbstractAxis.this);
        private final AxisLabelFormatter defaultTimeFormatter =
            new DefaultTimeFormatter(AbstractAxis.this);
  ...
  @Override
  public void drawAxis(final GraphicsContext gc, final double axisWidth,
    ...
    if (!isTickMarkVisible()) {
      ...
      drawAxisLabel(gc, axisWidth, axisHeight, getAxisLabel(), null,
                    getTickLength());
      drawAxisPost();
      return;
    }
    ...
    final double neededLength =
        (getTickMarks().size() + minorTicks.size()) * 2.0;
    // Don't draw minor tick marks if there isn't enough space for them!
    if (isMinorTickVisible() && (axisLength > neededLength)) {
      drawTickMarks(gc, axisLength, axisWidth, axisHeight, minorTicks,
                    getMinorTickLength(), getMinorTickStyle());
      drawTickLabels(gc, axisWidth, axisHeight, minorTicks,
                     getMinorTickLength());
    }
    ...
    // visible
    drawTickMarks(gc, axisLength, axisWidth, axisHeight, majorTicks,
                  getTickLength(), getMajorTickStyle());
    drawTickLabels(gc, axisWidth, axisHeight, majorTicks, getTickLength());

    // draw axis title
    drawAxisLabel(gc, axisWidth, axisHeight, getAxisLabel(), majorTicks,
                  getTickLength());
    drawAxisPost();
  }
  ...
  /**
   * Get the display position along this axis for a given value. If the value is
   * not in the current range, the returned value will be an extrapolation of
  ...
  public TickMark getNewTickMark(final double tickValue,
                                 final double tickPosition,
                                 final String tickMarkLabel) {
    TickMark tick;
    if (tickMarkLabel.isEmpty()) {
      // usually a minor tick mark w/o label
      tick = tickMarkDoubleCache.computeIfAbsent(
          tickValue,
          k
          -> new TickMark(getSide(), tickValue, tickPosition,
                          getTickLabelRotation(), ""));
    } else {
      // usually a major tick mark with label
      tick = tickMarkStringCache.computeIfAbsent(
          tickMarkLabel,
          k
          -> new TickMark(getSide(), tickValue, tickPosition,
                          getTickLabelRotation(), k));
  ...
  /**
   * Called when data has changed and the range may not be valid any more. This
   * is only called by the chart if isAutoRanging() returns true. If we are auto
   * ranging it will cause layout to be requested and auto ranging to happen on
   * next layout pass.
   *
  ...
  public void recomputeTickMarks() { recomputeTickMarks(getRange()); }

  /**
   * Request that the axis is laid out in the next layout pass. This replaces
   * requestLayout() as it has been overridden to do nothing so that changes to
   * children's bounds etc do not cause a layout. This was done as a
   * optimisation as the Axis knows the exact minimal set of changes that really
   * need layout to be updated. So we only want to request layout then, not on
   * any child change.
   */
  @Override
   ...
   * changing any properties of the axis. Any results of the auto-ranging should
   * be returned in the range object. This will we passed to setRange() if it
   * has been decided to adopt this range for this axis.
   *
  ...
  protected AxisRange autoRange(final double length) {
    ...
    if (isAutoRanging() || isAutoGrowRanging()) {
      ...
      final double labelSize = getTickLabelFont().getSize() *
                               1.2; // N.B. was '2' in earlier implementations
      return autoRange(autoRange.getMin(), autoRange.getMax(), length,
                       labelSize);
    }
    return getAxisRange();
  }
   ...
   *     length
   */
  protected abstract List<Double> calculateMajorTickValues(double length,
                                                           AxisRange range);
   ...
   */
  protected double calculateNewScale(final double length,
  ...
  protected void clearAxisCanvas(final GraphicsContext gc, final double width,
                                 final double height) {
   ...
   * orientation is horizontal, it takes into account the tick mark length, tick
  ...
  protected double computePrefHeight(final double width) {
    ...
    final double maxLabelHeightLocal =
        isTickLabelsVisible() ? maxLabelHeight : 0.0;
    ...
    final double tickMarkLength =
        isTickMarkVisible() && (getTickLength() > 0) ? getTickLength() : 0;
    // calculate label height
    final double labelHeight =
        (axisLabel.getText() == null) || axisLabel.getText().isEmpty()
            ? 0
            : axisLabel.prefHeight(-1) + (2 * getAxisLabelGap());
    final double shiftedLabels =
        ((getOverlapPolicy() == AxisLabelOverlapPolicy.SHIFT_ALT) &&
         isLabelOverlapping()) ||
  ...
  protected double computePrefWidth(final double height) {
    ...
    final double maxLabelWidthLocal =
        isTickLabelsVisible() ? maxLabelWidth : 0.0;
  ...
  /**
   * Computes range of this axis, similarly to {@link #autoRange(double, double,
   * double, double)}. The major difference is that this method is called when
   * {@link #autoRangingProperty() auto-range} is off.
   *
   ...
   */
  protected abstract AxisRange computeRange(double minValue, double maxValue,
                                            double axisLength,
                                            double labelSize);

  protected List<TickMark> computeTickMarks(final AxisRange range,
                                            final boolean majorTickMark) {
    ...
    final List<Double> newTickValues =
        majorTickMark ? calculateMajorTickValues(axisLength, range)
    ...
    newTickValues.forEach(newValue -> {
      ...
      final TickMark tick = getNewTickMark(
          newValue, tickPosition,
      ...
      if (majorTickMark && shouldAnimate()) {
        final FadeTransition ft =
            new FadeTransition(Duration.millis(750), tick);
  ...
  protected void drawAxisLabel(final GraphicsContext gc, final double axisWidth,
                               final double axisHeight, final Text axisName,
                               final ObservableList<TickMark> tickMarks,
    ...
    case LEFT: {
      ...
      final double x = axisWidth - tickLength - (2 * tickLabelGap) -
                       tickLabelSize - axisLabelGap - shiftedLabels;
    ...
    case RIGHT: {
      ...
      axisName.setRotate(-90);
      final double x = tickLength + tickLabelGap + tickLabelSize +
                       axisLabelGap + shiftedLabels;
      final double y = ((1.0 - labelPosition) * axisHeight) - labelGap;
      drawAxisLabel(gc, x, y, axisName);
    } break;
    ...
    case TOP: {
      ...
      final double y = axisHeight - tickLength - tickLabelGap - tickLabelSize -
                       axisLabelGap - shiftedLabels;
      drawAxisLabel(gc, x, y, axisName);
    } break;
    ...
    case BOTTOM: {
      gc.setTextBaseline(VPos.TOP);
      final double x = (labelPosition * axisWidth) + labelGap;
      final double y = tickLength + tickLabelGap + tickLabelSize +
                       axisLabelGap + shiftedLabels;
      drawAxisLabel(gc, x, y, axisName);
    } break;
    ...
    case CENTER_VER: {
      gc.setTextBaseline(VPos.TOP);
      axisName.setRotate(-90);
      final double x = (axisCentre * axisWidth) - tickLength -
                       (2 * tickLabelGap) - tickLabelSize - axisLabelGap -
                       shiftedLabels;
      final double y = ((1.0 - labelPosition) * axisHeight) - labelGap;
      drawAxisLabel(gc, x, y, axisName);
    ...
    case CENTER_HOR: {
      gc.setTextBaseline(VPos.TOP);
      final double x = (labelPosition * axisWidth) + labelGap;
      final double y = (axisCentre * axisHeight) + tickLength + tickLabelGap +
                       tickLabelSize + axisLabelGap + shiftedLabels;
      drawAxisLabel(gc, x, y, axisName);
    } break;
  ...
  protected void drawAxisLine(final GraphicsContext gc, final double axisLength,
                              final double axisWidth, final double axisHeight) {
    ...
    case LEFT:
      ...
      gc.strokeLine(snap(axisWidth), snap(0), snap(axisWidth),
                    snap(axisLength));
      break;
    case RIGHT:
    ...
    case TOP:
      ...
      gc.strokeLine(snap(0), snap(axisHeight), snap(axisLength),
                    snap(axisHeight));
      break;
    case BOTTOM:
    ...
    case CENTER_HOR:
      ...
      gc.strokeLine(snap(0), axisCentre * axisHeight, snap(axisLength),
                    snap(axisCentre * axisHeight));

      break;
    case CENTER_VER:
      // axis line at the centre of the canvas
      gc.strokeLine(snap(axisCentre * axisWidth), snap(0),
                    snap(axisCentre * axisWidth), snap(axisLength));
   ...
   */
  protected void drawAxisPost() { // NOPMD by rstein function can but does not
   ...
   */
  protected void drawAxisPre() { // NOPMD by rstein function can but does not
                                 // have to be overwritten
                                 // to be overwritten in derived classes
  }

  protected void drawTickLabels(final GraphicsContext gc,
                                final double axisWidth, final double axisHeight,
                                final ObservableList<TickMark> tickMarks,
                                final double tickLength) {
      ...
      for (final TickMark tickMark : tickMarks) {
        ...
        case SHIFT_ALT:
          if (isLabelOverlapping()) {
            x -= ((counter % 2) * tickLabelGap) +
                 ((counter % 2) * tickMark.getFont().getSize());
          }
          drawTickMarkLabel(gc, x, y, scaleFont, tickMark);
          break;
        ...
        default:
          if (((counter % 2) == 0) || !isLabelOverlapping() ||
              scaleFont < MAX_NARROW_FONT_SCALE) {
            drawTickMarkLabel(gc, x, y, scaleFont, tickMark);
          }
          break;
        }
  ...
  protected void drawTickMarks(final GraphicsContext gc,
                               final double axisLength, final double axisWidth,
                               final double axisHeight,
                               final ObservableList<TickMark> tickMarks,
                               final double tickLength, final Path tickStyle) {
    if (tickLength <= 0) {
      return;
    }
    final double paddingX = getSide().isHorizontal() ? getAxisPadding() : 0.0;
  ...
  protected AxisRange getAxisRange() {
    ...
    return new AxisRange(getMin(), getMax(), getLength(), getScale(),
                         getTickUnit());
  }
   ...
   */
  protected boolean isRangeValid() { return rangeValid; }
  ...
  protected void layoutChildren() {
    ...
    if (lengthDiffers || rangeInvalid || measureInvalid ||
      ...
      for (final TickMark m : majorTickMarks) {
        ...
        if (m.isVisible()) {
          final double tickSize = side.isHorizontal()
                                      ? m.getWidth()
      ...
      // '+1' tick label more because first and last tick are half outside axis
      // length
      final double projectedLengthFromIndividualMarks =
          (majorTickMarks.size() + 1) * maxLabelSize;

      switch (getOverlapPolicy()) {
      case NARROW_FONT:
        final double scale = axisLength / projectedLengthFromIndividualMarks;
        if ((scale >= MIN_NARROW_FONT_SCALE) &&
            (scale <= MAX_NARROW_FONT_SCALE)) {
          scaleFont = scale;
          break;
        }
        scaleFont = Math.min(Math.max(scale, MIN_NARROW_FONT_SCALE),
      ...
      case SKIP_ALT:
        ...
        if ((maxLabelSize > 0) && (axisLength < totalLabelsSize)) {
          numLabelsToSkip =
              (int)(projectedLengthFromIndividualMarks / axisLength);
          labelOverlap = true;
        }
  ...
  protected double measureTickMarkLength(final double major) {
    ...
    final TickMark tick =
        getNewTickMark(major, 0.0 /* NA */, getTickMarkLabel(major));
  ...
  protected void setRange(final AxisRange rangeObj, final boolean animate) {
    ...
    if (animate) {
      ...
      animator.getKeyFrames().setAll(
          ...
          new KeyFrame(
              Duration.millis(AbstractAxis.RANGE_ANIMATION_DURATION_MS),
  ...
  /**
   * This is used to check if any given animation should run. It returns true if
   * animation is enabled and the node is visible and in a scene.
   *
   ...
   */
  protected void tickMarksUpdated() { // NOPMD by rstein function can but does
   ...
   * efficiency reasons
   */
  protected void updateCachedVariables() { // NOPMD by rstein function can but
  ...
  protected void updateCSS() {
    ...
    final double diffMillisSinceLastUpdate =
        TimeUnit.NANOSECONDS.toMillis(now - lastCssUpdate);
    if (diffMillisSinceLastUpdate < AbstractAxis.BURST_LIMIT_CSS_MS) {
      if (!callCssUpdater) {
        callCssUpdater = true;
        // repaint 20 ms later in case this was just a burst operation
        final KeyFrame kf1 =
            new KeyFrame(Duration.millis(20), e -> requestLayout());
   ...
   */
  private static boolean isTickLabelsOverlap(final Side side, final TickMark m1,
                                             final TickMark m2,
                                             final double gap) {
    ...
    return side.isVertical() ? (m1Start - m2End) <= gap
                             : (m2Start - m1End) <= gap;
  }

  protected static void drawAxisLabel(final GraphicsContext gc, final double x,
                                      final double y, final Text label) {
    gc.save();
  ...
  protected static void drawTickMarkLabel(final GraphicsContext gc,
                                          final double x, final double y,
                                          final double scaleFont,
  ...
  getMaxTickLabelHeight(final List<TickMark> tickMarks) {
    return (tickMarks == null) || tickMarks.isEmpty()
        ? 0.0
        : tickMarks.stream()
              .mapToDouble(TickMark::getHeight)
              .max()
              .getAsDouble();
  }
