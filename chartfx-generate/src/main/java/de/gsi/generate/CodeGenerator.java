...
public class CodeGenerator extends AbstractMojo {
    ...
    private void addDataTypeVariants(final Path inputPath, final Path outputPath) throws MojoExecutionException {
        ...
        for (final Path source : sourcesTypes) {
            ...
            // original type
            final String outputClassSuffixOriginal = capitalise(inputType);
            final String outputClassNameOriginal = outputClassNameBase + outputClassSuffixOriginal;
            final Path destOriginal = outputDirectory.resolve(outputClassNameOriginal + JAVA_FILE_SUFFIX);
            try (Writer writer = Files.newBufferedWriter(destOriginal, CREATE, WRITE, TRUNCATE_EXISTING)) {
                writer.write(AUTOGENERATED_WARNING_HEADER);
                writer.write("// autogenerated code for " + inputType);
                writeLines(sourceContent, writer, line -> processLine(line, inputType, inputType).replace(PROTOTYPE_CLASS_SUFFIX, outputClassSuffixOriginal));
            } catch (IOException e) {
