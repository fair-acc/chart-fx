...
public class CmwLightClient {
    ...
    private byte[] checkHeader(final ZFrame headerMsg) throws RdaLightException {
        if (headerMsg == null) {
    ...
    private byte[] checkDescriptor(final ZFrame descriptorMsg, final ZFrame firstFrame) throws RdaLightException {
    ...
    private Reply createSubscriptionExceptionReply(final long id, final ZFrame exceptionBody) throws RdaLightException {
        ...
        reply.id = id;
        serialiser.setDataBuffer(FastByteBuffer.wrap(exceptionBody.getData()));
