...
class DataSetUtilsTest {
    ...
    void serializeAndDeserializeDataSet3D(boolean binary, boolean useFloat) {
        ...
        DataSet dataSet = new DataSetBuilder("Test 3D Dataset") //
                                  .setValues(DataSet.DIM_X, new double[] { 1.0f, 2.0f, 3.0f }) //
                                  .setValues(DataSet.DIM_Y, new double[] { 0.001f, 4.2f })
                                  .setValues(DataSet.DIM_Z, new double[][] { { 1.3f, 3.7f, 4.2f }, { 2.3f, 1.8f, 5.0f } }) //
                                  .setAxisName(DataSet.DIM_X, "U")
                                  .setAxisUnit(DataSet.DIM_X, "V") //
                                  .setAxisName(DataSet.DIM_Y, "I")
                                  .setAxisUnit(DataSet.DIM_Y, "A") //
                                  .setAxisName(DataSet.DIM_Z, "P")
                                  .setAxisUnit(DataSet.DIM_Z, "W") //
                                  .build();
    ...
    void serializeAndDeserializeDefaultDataSet(boolean binary, boolean useFloat) {
        ...
        DataSet dataSet = new DataSetBuilder() //
                                  ...
                                  .setValuesNoCopy(DataSet.DIM_X, new double[] { 1.0f, 2.0f, 3.0f, 4.0f, 5.0f }) //
                                  .setValuesNoCopy(DataSet.DIM_Y, new double[] { 1.3f, 3.7f, 4.2f, 2.3f, 1.8f }) //
                                  .setPosErrorNoCopy(DataSet.DIM_Y, new double[] { 0.1f, 0.3f, 0.2f, 0.3f, 0.8f }) //
                                  .setNegErrorNoCopy(DataSet.DIM_Y, new double[] { 0.1f, 0.3f, 0.2f, 0.3f, 0.8f }) // DataSetUtils defaults to ASYMMETRIC
                                  .setAxisName(DataSet.DIM_X, "index")
                                  .setAxisUnit(DataSet.DIM_X, "") //
                                  .setAxisName(DataSet.DIM_Y, "Voltage")
                                  .setAxisUnit(DataSet.DIM_Y, "V") //
        ...
        for (int dim = 0; dim < dataSet.getDimension(); dim++) {
            ...
            assertArrayEquals( //
                    ...
                    EPSILON, msg);
            if (dataSet instanceof DataSetError && ((DataSetError) dataSet).getErrorType(dim) != DataSetError.ErrorType.NO_ERROR) {
                assertArrayEquals( //
    ...
    void testEvaluateCompression() {
        Assertions.assertEquals(DataSetUtils.Compression.GZIP, DataSetUtils.evaluateAutoCompression("test.bin.gz"));
        Assertions.assertEquals(DataSetUtils.Compression.ZIP, DataSetUtils.evaluateAutoCompression("test.csv.zip"));
        Assertions.assertEquals(DataSetUtils.Compression.NONE, DataSetUtils.evaluateAutoCompression("test.csv"));
    }
    ...
    void testSplitCharByteInputStream() throws IOException {
        final byte[] byteArray = new byte[] { 'a', 'b', 'c', DataSetUtils.SplitCharByteInputStream.MARKER, 120, 96 };
        DataSetUtils.SplitCharByteInputStream scbiStream = new DataSetUtils.SplitCharByteInputStream(new PushbackInputStream(new ByteArrayInputStream(byteArray)));
        // single byte method
        ...
        assertEquals(-1, scbiStream.read());
        // chunk methods
        scbiStream = new DataSetUtils.SplitCharByteInputStream(new PushbackInputStream(new ByteArrayInputStream(byteArray)));
        byte[] output = new byte[2];
        assertEquals(2, scbiStream.read(output));
        assertArrayEquals(new byte[] { 'a', 'b' }, output);
        assertFalse(scbiStream.reachedSplit());
        assertEquals(1, scbiStream.read(output));
        assertArrayEquals(new byte[] { 'c', DataSetUtils.SplitCharByteInputStream.MARKER }, output);
        assertTrue(scbiStream.reachedSplit());
        assertEquals(-1, scbiStream.read(output));
        scbiStream.switchToBinary();
        assertFalse(scbiStream.reachedSplit());
        ...
        assertArrayEquals(new byte[] { 0, 'a', 'b', 'c', DataSetUtils.SplitCharByteInputStream.MARKER, 0, 0, 0 }, output);
        assertTrue(scbiStream.reachedSplit());
    ...
    void testGenerateFileName() {
        ...
        final DataSet dataSet = new DataSetBuilder("dsName") //
                                        ...
                                        .setValuesNoCopy(DataSet.DIM_Y, new double[] { 9, 7, 8 }) //
