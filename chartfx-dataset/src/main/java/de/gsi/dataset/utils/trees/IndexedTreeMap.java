 ...
 * </p>
 * <p>
 * This implementation provides guaranteed log(n) time cost for the <code>containsKey</code>, <code>get</code>,
 * <code>put</code> and <code>remove</code> operations. Algorithms are adaptations of those in Cormen, Leiserson, and
 ...
 * <code>Comparable</code> or <code>Comparator</code> for a precise definition of <i>consistent with equals</i>.) This
 * is so because the <code>Map</code> interface is defined in terms of the equals operation, but a map performs all key
 * comparisons using its <code>compareTo</code> (or <code>compare</code>) method, so two keys that are deemed equal by
 * this method are, from the standpoint of the sorted map, equal. The behaviour of a sorted map <i>is</i> well-defined
 * even if its ordering is inconsistent with equals; it just fails to obey the general contract of the <code>Map</code>
 * interface.
 * </p>
 * <p>
 ...
 * created, in any way except through the iterator's own <code>remove</code> method, the iterator will throw a
 ...
 * that depended on this exception for its correctness: <i>the fail-fast behaviour of iterators should be used only to
 * detect bugs.</i>
 * </p>
 * <p>
 * All <code>Map.Entry</code> pairs returned by methods in this class and its views represent snapshots of mappings at
 * the time they were produced. They do <em>not</em> support the <code>Entry.setValue</code> method. (Note however that
 * it is possible to change mappings in the associated map using <code>put</code>.)
 * </p>
 * <p>
...
public class IndexedTreeMap<K, V> extends AbstractMap<K, V> implements IndexedNavigableMap<K, V>, Cloneable, java.io.Serializable {
     ...
     * <code>k1.compareTo(k2)</code> must not throw a <code>ClassCastException</code> for any keys <code>k1</code> and
     * <code>k2</code> in the map. If the user attempts to put a key into the map that violates this constraint (for
     ...
     * k2)</code> must not throw a <code>ClassCastException</code> for any keys <code>k1</code> and <code>k2</code> in
     ...
     * <code>ClassCastException</code> for any keys <code>k1</code> and <code>k2</code> in the map. This method runs in
     * n*log(n) time.
     *
    ...
    /**
     * Returns a shallow copy of this <code>IndexedTreeMap</code> instance. (The keys and values themselves are not
     * cloned.)
     *
     ...
     * map size for most implementations.
     *
    ...
    /**
     * @throws ClassCastException {@inheritDoc}
     * @throws NullPointerException if <code>toKey</code> is null and this map uses natural ordering, or its comparator
     *         does not permit null keys
     ...
     * <code>remove</code> operation), the results of the iteration are undefined. The set supports element removal,
     ...
     *         <code>key</code>. (A <code>null</code> return can also indicate that the map previously associated
    ...
    /**
     * @throws ClassCastException {@inheritDoc}
     * @throws NullPointerException if <code>fromKey</code> or <code>toKey</code> is null and this map uses natural
    ...
    /**
     * @throws ClassCastException {@inheritDoc}
     * @throws NullPointerException if <code>fromKey</code> is null and this map uses natural ordering, or its
    ...
    protected static final class Entry<K, V> implements Map.Entry<K, V> {
        ...
        protected boolean color = IndexedTreeMap.BLACK;
    ...
    static abstract class NavigableSubMap<K, V> extends java.util.AbstractMap<K, V>
        ...
        protected abstract IndexedTreeMap.Entry<K, V> subCeiling(K key);

        protected abstract IndexedTreeMap.Entry<K, V> subFloor(K key);

        protected abstract IndexedTreeMap.Entry<K, V> subHigher(K key);

        protected abstract IndexedTreeMap.Entry<K, V> subHighest();

        protected abstract IndexedTreeMap.Entry<K, V> subLower(K key);

        protected abstract IndexedTreeMap.Entry<K, V> subLowest();
        ...
        protected abstract class EntrySetView extends AbstractSet<Map.Entry<K, V>> {
            ...
            public int size() {
                ...
                if (size == -1 || sizeModCount != m.modCount) {
                    ...
                    size = 0;
                    for (final Entry<K, V> kvEntry : this) {
                        size++;
                    }
                }
                return size;
        ...
        protected abstract class SubMapIterator<T> implements Iterator<T> {
            ...
            protected SubMapIterator(IndexedTreeMap.Entry<K, V> first, IndexedTreeMap.Entry<K, V> fence) {
            ...
            public final IndexedTreeMap.Entry<K, V> nextEntry() {
            ...
            public final IndexedTreeMap.Entry<K, V> prevEntry() {
                final IndexedTreeMap.Entry<K, V> e = next;
                if (e == null || e.key == fenceKey) {
                    throw new NoSuchElementException();
                }
