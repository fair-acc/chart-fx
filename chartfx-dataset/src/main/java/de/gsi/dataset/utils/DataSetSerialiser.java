...
public class DataSetSerialiser extends DataSetUtilsHelper {
	...
	protected static Optional<FieldHeader> checkFieldCompatibility(final IoBuffer buffer, final List<FieldHeader> fieldHeaderList,
			final String fieldName, final DataType ...requireDataTypes) {
		Optional<FieldHeader> fieldHeader = FieldHeader.findHeaderFor(fieldHeaderList, DATA_SET_NAME);
...
//		if (checkFieldCompatibility(readBuffer, fieldHeaderList, VAL_INTEGRAL, DataType.DOUBLE).isPresent()) {
//			BinarySerialiser.getDouble(readBuffer); // not used for the moment
//		}
//
//		if (checkFieldCompatibility(readBuffer, fieldHeaderList, VAL_MEAN, DataType.DOUBLE).isPresent()) {
//			BinarySerialiser.getDouble(readBuffer); // not used for the moment
//		}
//
//		if (checkFieldCompatibility(readBuffer, fieldHeaderList, VAL_RMS, DataType.DOUBLE).isPresent()) {
//			BinarySerialiser.getDouble(readBuffer); // not used for the moment
//		}
	}
