...
public class FastByteBuffer implements IoBuffer {
	...
	/**
	 * Forces FastByteBUffer to contain the given number of entries, preserving just
	 * a part of the array.
	 *
	...
	/**
	 * Trims the internal buffer array if it is too large. If the current array
	 * length is smaller than or equal to {@code n}, this method does nothing.
	 * Otherwise, it trims the array length to the maximum between
	 * {@code requestedCapacity} and {@link #capacity()}.
	 * <p>
	 * This method is useful when reusing FastBuffers. {@linkplain #reset() Clearing
	 * a list} leaves the array length untouched. If you are reusing a list many
